...
var jsonUtils = function() {
    ...
    * @return {Array} 
    */
    this.getJsonTree = function(jsonObj) {
		var jsonTree = [];
			...
			if(_.isArray(value)) {
				...
				_.each(value, function(arrayValues, arrayIndex) {
					if(!_.isObject(arrayValues)) {
						treeNode.nodes.push({'text': arrayIndex, 'nodes': new Array({'text': arrayValues})});	
					} else {
						treeNode.nodes.push({'text': arrayIndex, 'nodes': this.getJsonTree(arrayValues)});	
					}
					
				});
			} else if (_.keys(value).length > 0) {
				treeNode.text = treeNode.text + " {Object}";
				treeNode.nodes = this.getJsonTree(value);
			} else {
				treeNode.nodes.push({'text': value});
			}
