 ...
 * @return {Array}
 */
jsonsql.prototype.query = function(columns, tablePath, whereClauseFn) {
	this.currentTable = null;
	var table = jsonUtils.getJsonNode(this.db, tablePath), queryResult;
...
var jsonUtils = function() {
    ...
    */
	...
	// given a table schema, generate column suggestions
	/**
    * Create an array of suggestions based on the schema
    * @param  {Object} nodeSchema
	* @param  {String} nodeRootKey
	* @param  {Boolean} excludeLeaf - exclude leaf nodes (nodes that are not objects or arrays) . Should be set to true to be used to generate suggestions for table names
    * @return {Array} 
    */
	this.generateSuggestions = function(nodeSchema, nodeRootKey, excludeLeaf) {
		var suggestions = [];
		if (_.isUndefined(nodeSchema)) {
			return suggestions;
		}

		//if excludeLeaf is true, we are generating suggestions for table names
		//and we need to exclude leaf nodes
		if (excludeLeaf && (nodeSchema.type !== 'object' && nodeSchema.type !== 'array')) {
			return suggestions;
		}
		
		if (nodeSchema.type === 'array' && !_.isUndefined(nodeSchema.items) && !_.isUndefined(nodeSchema.items.properties)) {
			suggestions = Object.keys(nodeSchema.items.properties)
				.map(function(key) {
					var prop = nodeSchema.items.properties[key];
					if (excludeLeaf && prop.type !== 'object' && prop.type !== 'array') {
						return;
					}
					return nodeRootKey ? nodeRootKey + '.' + key : key;
				})
				.filter(function(v) { return typeof v !== 'undefined'; });
			// also add nested object properties
			for (var key in nodeSchema.items.properties) {
				if (nodeSchema.items.properties.hasOwnProperty(key)) {
					var prop = nodeSchema.items.properties[key];
					suggestions = suggestions.concat(this.generateSuggestions(prop, nodeRootKey ? nodeRootKey + '.' + key : key, excludeLeaf));
				}
			}
		} else if (nodeSchema.type === 'object' && !_.isUndefined(nodeSchema.properties)) {
			suggestions = Object.keys(nodeSchema.properties)
				.map(function(key) {
					var prop = nodeSchema.properties[key];
					if (excludeLeaf && prop.type !== 'object' && prop.type !== 'array') {
						return;
					}
					return nodeRootKey ? nodeRootKey + '.' + key : key;
				...
				.filter(function(v) { return typeof v !== 'undefined'; });
			// also add nested object properties
			for (var key in nodeSchema.properties) {
				if (nodeSchema.properties.hasOwnProperty(key)) {
					var prop = nodeSchema.properties[key];
					suggestions = suggestions.concat(this.generateSuggestions(prop, nodeRootKey ? nodeRootKey + '.' + key : key, excludeLeaf));
				}
			}
	...
	/**
	 * Given a table name, and its json schema, return first level column names for that table
	 * @param  {String} tableName
	 * @param  {Object} jsonSchema
	 * @return {Array}
	 */
	this.getColumnNamesFromSchema = function(tableName, jsonSchema) {
		var tableSchema = this.getSchemaNodeByPath(jsonSchema, tableName);
