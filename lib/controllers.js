...
jsonSqlControllers.controller('QueryDBCtrl', ['$scope', '$location', 'appContext', '$filter', function($scope, $location, appContext, $filter) {
	...
	function getTableSuggestions(jsonSchema) {
		var tableNames = [];
		var jsonSchema = appContext.getJsonSchema();
		if (_.isUndefined(jsonSchema) || _.isNull(jsonSchema)) {
			console.log('json schema is not defined');
			return;
		}
		$scope.tableSuggestions = jsonUtils.generateSuggestions(jsonSchema,undefined, true);
		
	}
	getTableSuggestions(jsonSchema);
	...
	function getColumnSuggestions() {
		...
		if (isTableDefined && isSchemaDefined) {
			var tableSchema = jsonUtils.getSchemaNodeByPath(schema, $scope.table);
			columnSuggestions = jsonUtils.generateSuggestions(tableSchema);
		}
	...
	if ($scope.jsonDB.db.dataArray) {
		...
		for (const [key, value] of Object.entries(db)) {
			if (db[key] != null && !Array.isArray(db[key]) && typeof db[key] === 'object') {
				$scope.table = key;
				break;
			}
		}
	...
	//now that we have default table selected get all columns for that table based on the schema
	// and query the table
	var columns = jsonUtils.getColumnNamesFromSchema($scope.table, appContext.getJsonSchema());
	$scope.columns = columns;
	$scope.queryResult = jsonDB.query(columns, $scope.table, undefined);
...
jsonSqlControllers.factory('appContext', function() {
	return {
		...
		setJsonDB: function(jsonObj) {
			...
			this.jsonSchema = jsonUtils.generateJsonSchema(jsonObj);
		},
		getJsonDB: function() {
			return this.jsonDB;
		},
