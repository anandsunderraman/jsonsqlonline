var jsonSqlControllers = angular.module('jsonSqlControllers', ['jsonFormatter','ui.bootstrap']);
...
jsonSqlControllers.controller('CreateDBCtrl', ['$scope', '$location', 'appContext', function($scope, $location, appContext) {
  ...
  $scope.createDB = function() {
  	try {
		...
		// var jsonSchema = toJsonSchema(userJSONData);
...
jsonSqlControllers.controller('QueryDBCtrl', ['$scope', '$location', 'appContext', '$filter', function($scope, $location, appContext, $filter) {
	...
	function getTableSuggestions() {
		var tableNames = [];
		var jsonSchema = appContext.getJsonSchema();
		if (!_.isUndefined(jsonSchema) && !_.isNull(jsonSchema)) {
			if (jsonSchema.properties) {
				tableNames = Object.keys(jsonSchema.properties);
			}
		}
		$scope.tableSuggestions = tableNames;
	...
	function getColumnSuggestions() {
		...
		var isTableDefined = !_.isUndefined($scope.table) && $scope.table !== "";
		var schema = appContext.getJsonSchema();
		var isSchemaDefined = !_.isUndefined(schema) && !_.isNull(schema);

		if (isTableDefined && isSchemaDefined) {
			//check the definition of table in schema
			//if it is of type array get the items properties
			//else get the properties of the object
			var tableSchema = jsonUtils.getJsonNode(schema.properties, $scope.table);
			if (!_.isUndefined(tableSchema)) {
				if (tableSchema.type === 'array' && !_.isUndefined(tableSchema.items) && !_.isUndefined(tableSchema.items.properties)) {
					columnSuggestions = Object.keys(tableSchema.items.properties);
				} else if (tableSchema.type === 'object' && !_.isUndefined(tableSchema.properties)) {
					columnSuggestions = Object.keys(tableSchema.properties);
				}
			}
		}
		// $scope.columNames = columnNames;
		console.log(columnSuggestions);
		$scope.columnSuggestions = columnSuggestions;
	}
	$scope.getColumnSuggestions = getColumnSuggestions;
	...
	$scope.addColumn = function(event) {
		if (event.keyCode === 188 || event.keyCode === 13) { // comma or Enter
			...
			if ($scope.columnInput && $scope.columnInput.trim()) {
				$scope.columns.push($scope.columnInput.trim());
				$scope.columnInput = '';
			}
		}
	};

	$scope.onColumnSelect = function(item) {
        if (!item) return;
        
        // Add to columns array if not already there
        if ($scope.columns.indexOf(item) === -1) {
            $scope.columns.push(item);
        }
        
        // Clear the input field for next selection
        $scope.columnInput = '';
    };
	...
	$scope.populateSelect = function() {
		var selectedNodes = $('.jsonTree').treeview('getSelected');
		if (!_.isUndefined(selectedNodes)) {
			var columnName = selectedNodes[0].jsonPath;
			if ($scope.columns.indexOf(columnName) === -1) {
...
jsonSqlControllers.factory('appContext', function() {
	return {
		...
		setJsonDB: function(jsonObj) {
			...
			this.jsonSchema = toJsonSchema(jsonObj, {arrays: {mode: 'all'}});
		},
		getJsonDB: function() {
			return this.jsonDB;
		},
